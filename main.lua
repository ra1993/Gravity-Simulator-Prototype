...
function spaceclick(event)
   ...
   transition.to(space, {x = 350, y = 545, xScale = 2, yScale = 2})
   
local spaceInfo = display.newText("Space's Gravity: 0m/s² ", w/2, h/2, Arial, 50)
...
physics.setGravity(0, 0)  -- applying gravity on object
local crate = display.newRect(100, 50, 40, 40)
crate:setFillColor(1, .5, 0)
crate.x = math.random(500)
crate.y = math.random(500)
crate.rotation = 5
crate.gravityScale = .25
physics.addBody (crate, {density = 3.0, friction = .5, bounce = 1, "dynamic"})

end


space:addEventListener("tap", spaceclick)
